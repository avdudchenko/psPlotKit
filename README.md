...
Expressions let you derive new data from already-imported keys using Python arithmetic operators. Instead of writing expression strings, you build expression trees with the ExpressionNode / ExpressionKeys API.
...
Each entry in the manager is a PsData object that wraps a numpy array with physical units. You can retrieve data by directory and key, and perform arithmetic on PsData objects directly.
